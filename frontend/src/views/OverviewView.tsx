...
export function OverviewView() {
  ...
  return <>
    ...
    <div className="mt-4 grid grid-cols-3 gap-4"><UsageChart points={data.timeline}/><div className="panel p-5"><h3 className="text-sm font-semibold">Harnesses</h3><p className="mt-1 text-xs text-zinc-500">Latest detected activity</p><div className="mt-5 space-y-5">{Object.entries({claude:'Claude Code',codex:'Codex',cursor:'Cursor',kiro:'Kiro'}).map(([id,name]) => { const item=data.providers.find(p=>p.provider===id); return <div key={id} className="flex items-center gap-3"><ProviderMark provider={id}/><div className="min-w-0 flex-1"><div className="flex items-center justify-between"><span className="text-sm font-medium" style={{color:providerColors[id]}}>{name}</span><span className="text-xs text-zinc-400">{compactNumber(item?.tokens ?? 0)}</span></div><div className="mt-1 flex justify-between text-[11px] text-zinc-600"><span className="truncate">{item?.model ?? 'No model detected'}</span><span>{relativeTime(item?.last_active ?? null)}</span></div></div></div>})}</div></div></div>
