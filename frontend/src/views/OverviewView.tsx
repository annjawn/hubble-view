import { Clock3, FolderKanban, MousePointerClick, RefreshCw, Shapes } from 'lucide-react'
...
import { ModelUsageRail } from '../components/dashboard/ModelUsageRail'
...
export function OverviewView() {
  ...
  return <>
    ...
    <div className="grid grid-cols-4 gap-4"><MetricCard label="Total tokens" value={compactNumber(data.totals.total_tokens)} detail={`${compactNumber(data.totals.input_tokens)} fresh · ${compactNumber(data.totals.cache_read_tokens)} cached`} icon={Shapes}/><MetricCard label="Sessions" value={compactNumber(data.totals.sessions)} detail="Across the last 30 days" icon={Clock3}/><MetricCard label="Tool calls" value={compactNumber(data.totals.tool_calls)} detail="Detected local invocations" icon={MousePointerClick}/><MetricCard label="Active projects" value={compactNumber(data.totals.active_projects)} detail={`${data.totals.active_models} models across ${data.totals.active_harnesses} harnesses`} icon={FolderKanban}/></div>
    <div className="mt-4 grid grid-cols-3 gap-4"><UsageChart points={data.timeline}/><div className="panel p-5"><h3 className="text-sm font-semibold">Harnesses</h3><p className="mt-1 text-xs text-zinc-500">Latest detected activity</p><div className="mt-5 space-y-5">{Object.entries({claude:'Claude Code',codex:'Codex',cursor:'Cursor',kiro:'Kiro',opencode:'OpenCode',antigravity:'Antigravity'}).map(([id,name]) => { const item=data.providers.find(p=>p.provider===id); return <div key={id} className="flex items-center gap-3"><ProviderMark provider={id}/><div className="min-w-0 flex-1"><div className="flex items-center justify-between"><span className="text-sm font-medium" style={{color:providerColors[id]}}>{name}</span><span className="text-xs text-zinc-400">{compactNumber(item?.tokens ?? 0)}</span></div><div className="mt-1 flex justify-between text-[11px] text-zinc-600"><span>{item ? `${item.sessions} sessions` : 'No activity in 30 days'}</span><span>{relativeTime(item?.last_active ?? null)}</span></div></div></div>})}</div></div></div>
    <ModelUsageRail models={data.models}/>
