...
export function TrayView() {
  ...
  return <div className="min-h-screen bg-[#0d1015] p-4"><div className="drag-region flex items-center justify-between pb-4"><div><div className="text-sm font-semibold">Hubble</div><div className="mt-0.5 text-[11px] text-zinc-600">Local usage now</div></div><button className="no-drag grid h-8 w-8 place-items-center rounded-lg hover:bg-white/[0.06]" onClick={()=>scan.mutate()}><RefreshCw size={14} className={scan.isPending?'animate-spin text-indigo-400':'text-zinc-500'}/></button></div>
    ...
    <div className="mt-3 space-y-2">{Object.entries({claude:'Claude Code',codex:'Codex',cursor:'Cursor',kiro:'Kiro'}).map(([id,name])=>{const item=data?.providers.find(p=>p.provider===id);return <div key={id} className="flex items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.025] p-3"><ProviderMark provider={id} size="sm"/><div className="min-w-0 flex-1"><div className="flex justify-between text-xs"><span className="font-medium">{name}</span><span>{compactNumber(item?.tokens ?? 0)}</span></div><div className="mt-1 truncate text-[10px] text-zinc-600">{item?.model ?? 'No active model'}</div></div></div>})}</div>
